...
export function getSearchPlayer(input, sport) {
  return dispatch => {
    axios.get(`http://localhost:3001/player/getSearchPlayer?name=${input}`)
      .then(res => {
        dispatch({
...
export function getPlayersProfile(email) {
  return dispatch => {
    axios.get(`http://localhost:3001/player/getPlayerProfile/?email=${email}`)
      .then(res => {
        dispatch({
...
export function putPlayer(email, obj) {
  return dispatch => {
    axios.put(`http://localhost:3001/player/modifyProfile?email=${email}`, obj)
      .then(res => {
        dispatch({
