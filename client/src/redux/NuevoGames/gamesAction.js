...
export function getGameSport(sport) {
  return (dispatch) => {
    axios
      .get(`http://localhost:3001/games/${sport}`)
      .then((res) => {
...
export function getSearchGames(input, sport) {
  return (dispatch) => {
    axios
      .get(`http://localhost:3001/games/${sport}/searchGame?name=${input}`)
      .then((res) => {
        dispatch({
...
export function getGamesById(id) {
  return (dispatch) => {
    axios
      .get(`http://localhost:3001/games/detail/${id}`)
      .then((res) => {
        dispatch({
...
export function updateGame(id, body) {
  return dispatch => {
    axios.put(`http://localhost:3001/games/updateGame`, body)
      .then(res => {
        dispatch({
          type: UPDATE_GAME,
          payload: res.data
...
export const sendInvitation = (body) => {
  return async function (dispatch) {
    return axios.post("http://localhost:3001/sendGrid/invitation", body)
      .then((res) => {
...
export const gamesByUser = (email) => {
  return async function (dispatch) {
    return axios.get(`http://localhost:3001/games/misEventos/all/${email}`)
      .then((res) => {
