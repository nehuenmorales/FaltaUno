...
const PlayersAdmin = () => {
    ...
    const onClick = async (ev) => {
        await axios.put(`http://localhost:3001/player/modifyStatus/${ev.target.value}`, { status: 'banned' })
        //setCambio(ev.target.value)
        window.location.reload(true)
    }
    const onClickDesbloquear = async (ev) => {
        await axios.put(`http://localhost:3001/player/modifyStatus/${ev.target.value}`, { status: 'allowed' })
        //setCambio(ev.target.value)
        window.location.reload(true)
    }
