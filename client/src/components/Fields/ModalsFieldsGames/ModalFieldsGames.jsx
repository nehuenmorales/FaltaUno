...
export default function ModalsFieldsGames({ showModal, setShowModal, setNewField, newField, sport, convertirTime, fieldId, type }) {
    ...
    const modificar = async () => {
        setIndice(0)
        if (fieldId) {
            const respuesta = await axios.delete(`http://localhost:3001/owner/deleteGames/${fieldId}`)
        } else {
            const res = await axios.delete(`http://localhost:3001/owner/deleteField/${field.id}`)
        }
        setShowModal(false)
